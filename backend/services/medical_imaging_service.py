...
class MedicalImageProcessor:
    ...
    def _assess_image_quality(self, image: Image.Image) -> Dict:
        """Assess overall image quality for diagnostic purposes"""
        ...
        return {
            ...
            'recommendations': self._get_quality_recommendations(quality_score, noise_level)
        }
    
    def _get_quality_recommendations(self, quality_score: float, noise_level: str) -> List[str]:
        """Get recommendations for image quality improvement"""
        recommendations = []
        
        if quality_score < 0.4:
            recommendations.append("Image quality is poor - consider retaking")
        
        if noise_level == "High noise":
            recommendations.append("High noise detected - check imaging parameters")
        
        if quality_score < 0.6:
            recommendations.append("Image may not be optimal for detailed analysis")
            recommendations.append("Radiologist review strongly recommended")
